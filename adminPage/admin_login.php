<?php
    ...
    session_start();
    if(isset($_POST['insert_login'])){
        $email= $_POST['email'];
        $pass= $_POST['password'];
        $password = md5($pass);
        $checkExist="SELECT * FROM `admin_table` WHERE admin_email='$email' and admin_password='$password'";
        $resultCheck = mysqli_query($con, $checkExist);
        $ceckCount = mysqli_fetch_assoc($resultCheck);
        if($ceckCount > 0){
            $_SESSION['adminName'] = $ceckCount['admin_name'];
            $_SESSION['adminEmail'] = $email;
            $n = $ceckCount['admin_name'];
            echo "<script>alert('Login successfully welcome: ($n)')</script>";
            ...
            <div class="col-lg-6 col-xl-4 mt-3">    
                <form action="" method="post" enctype="multipart/form-data" class="mt-5">
                    ...
                    <div class="form-outline w-50 m-auto"> 
                        <input  type=submit class="btn btn-info p-2 px-5 border-0 text-light" name="insert_login" value="Login"> 
                        <p class="fw-bold p-2">Don`t have an account? <a href="admin_register.php" class="text-danger text-decoration-none">Register</a></p> 
                    </div>
                </form>
            </div>
        </div>
